    ...
    public HookResult OnPlayerConnectFull(EventPlayerConnectFull @event, GameEventInfo info)
    {
        ...
        _plugin.AddTimer(1.0f, () =>
        {
            ...
            if (!isInQueue && !isInActivePlayers)
            {
                ...
                {
                    // Check if player has bypass flag
                    var hasBypassFlag = PlayerHelper.HasAdminPermission(player, _plugin.Config.Queue.AutoAddToQueue.AutoAddToQueueBypassFlag.ToArray());
                    if (hasBypassFlag)
                    {
                        // Player has bypass flag - don't add to queue automatically (hideme support)
                        Logger.LogDebug("Player", $"[{player.PlayerName}] Player has bypass flag - not adding to queue automatically");
                        return;
                    }

                    if (!_plugin.Config.Queue.AutoAddToQueue.ShouldAutoAddToQueue)
                    {
                        return;
                    }
